...
public class Init{
	...
	public static void initAllStatics(){
		if(silver.langutil.pp.Init.preInit) return;

		silver.langutil.pp.Init.preInit = true;
	...
	public static void init(){
		if(silver.langutil.pp.Init.init) return;

		silver.langutil.pp.Init.init = true;

		silver.langutil.pp.Init.setupInheritedAttributes();

		silver.util.deque.Init.init();
		core.Init.init();

		silver.langutil.pp.Init.initProductionAttributeDefinitions();
	}

	public static void postInit(){
		if(silver.langutil.pp.Init.postInit) return;

		silver.langutil.pp.Init.postInit = true;
	...
	private static void setupInheritedAttributes(){
		...
		//	local attribute pr::Pair<Deque<Pair<Integer Decorated core:List<Boolean>>> Decorated core:List<Boolean>>;
		...
		//	local attribute le::Pair<Deque<Pair<Integer Decorated core:List<Boolean>>> Decorated core:List<Boolean>>;
	...
	private static void initProductionAttributeDefinitions(){
		...
		silver.langutil.pp.Pshow.childInheritedAttributes[silver.langutil.pp.Pshow.i_d][silver.langutil.pp.Init.silver_langutil_pp_inCHorizontals__ON__silver_langutil_pp_Document] = new common.Lazy() { public final Object eval(final common.DecoratedNode context) { return ((common.ConsCell)core.Pcons.invoke(new Object[]{false, context.childSynthesizedLazy(silver.langutil.pp.Pshow.i_d, silver.langutil.pp.Init.silver_langutil_pp_horizontals__ON__silver_langutil_pp_Document)})); } };
		// d.inRemaining = width
		...
		//FUNCTION concat Document ::= ds::Decorated core:List<Document> 
		//FUNCTION implode Document ::= sep::Document ds::Decorated core:List<Document> 
		//FUNCTION terminate Document ::= sep::Document ds::Decorated core:List<Document> 
		...
		//FUNCTION prune Pair<Deque<Pair<Integer Decorated core:List<Boolean>>> Decorated core:List<Boolean>> ::= p::Integer q::Deque<Pair<Integer Decorated core:List<Boolean>>> 
		//FUNCTION enter Deque<Pair<Integer Decorated core:List<Boolean>>> ::= p::Integer q::Deque<Pair<Integer Decorated core:List<Boolean>>> 
		//FUNCTION leave Pair<Deque<Pair<Integer Decorated core:List<Boolean>>> Decorated core:List<Boolean>> ::= p::Integer q::Deque<Pair<Integer Decorated core:List<Boolean>>> 
	}

	public static int count_local__ON__silver_langutil_pp_show = 0;
	public static int count_local__ON__silver_langutil_pp_concat = 0;
	public static int count_local__ON__silver_langutil_pp_implode = 0;
